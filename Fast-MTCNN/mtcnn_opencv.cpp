...
typedef struct StructPointerTest{
    int num;
    int location[2000]; // at most 500/one frame
...
//    vector<FaceInfo> faceInfo= detector.Detect_mtcnn(img,minSize, threshold, factor,2);
//    double et = (cv::getTickCount()-st)/cv::getTickFrequency();
...
//    }
//    cv::imwrite("images/test_out.jpg",img);
//	return 0;
//}

StructPointer detection(unsigned char *matrix,int width, int height, int channel)
{
    cv::Mat img;
    if(channel == 3)
    {
        cv::Mat image(width,height,CV_8UC3,matrix);
        img = image;
    }
    else if(channel == 1)
    {
        cv::Mat image(width,height,CV_8U,matrix);
        img = image;
    }
	...
	float threshold[3] = { 0.7f, 0.6f, 0.6f };
	int minSize = 12;
	vector<FaceInfo> info = detector.Detect_mtcnn(img,minSize, threshold, factor,2);
	int index = 0;

	for(int i = 0; i<info.size();i++)
	{
        p->location[index++] = (int)info[i].bbox.xmin;
        p->location[index++] = (int)info[i].bbox.ymin;
        p->location[index++] = (int)info[i].bbox.xmax;
        p->location[index++] = (int)info[i].bbox.ymax;
