...
class AnnotationTransform(object):
    ...
    def __call__(self, target, width, height):
        ...
        for i in range(num):
            ...
            ymax = int(target[4+i*4]) + ymin  # x y w h -> x1 y1 x2 y2
...
class Detection(data.Dataset):
    ...
    def __init__(self, anno_file, transform=None, target_transform=None,dataset_name='WiderFace'):
        self.anno_file = anno_file # None
    ...
    def pull_item(self, index):
        ...
        height, width, channels = img.shape # image.shape
       
        if self.target_transform is not None:
            target = self.target_transform(target, width, height)

        if self.transform is not None:
            target = np.array(target) # tensor-> numpy
            img, boxes, labels = self.transform(img, target[:, :4], target[:, 4]) # x,y,w,h confidence
