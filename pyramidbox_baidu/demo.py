...
def bbox_vote(det):
    ...
    dets = dets[0:200, :] #  detection'box threshold
    return dets
...
video_path = '/home/zhex/Videos/test_video/outdoor/middle_crowd.mp4'
...
#         #image = cv2.resize(frame, (width//4, height//4), interpolation=cv2.INTER_CUBIC) # 8ms 2160 3840
#         image = cv2.resize(frame, (160, 160), interpolation=cv2.INTER_CUBIC)
...
def main():
    ...
    image = cv2.resize(img, (160, 160), interpolation=cv2.INTER_CUBIC)
    max_im_shrink = (0x7fffffff / 200.0 / (image.shape[0] * image.shape[1])) ** 0.5  # 1.1377777775128681
